...
$page_title = "Kumpulan Hadits | Al-Huda";

// API configuration
$hadith_api_base = "https://api.hadith.gading.dev/books/";
$available_books = [
    'bukhari' => ['name' => 'Shahih Bukhari', 'total' => 7563],
...
// Get parameters
$selected_book = $_GET['kitab'] ?? 'bukhari';
$current_page = max(1, (int)($_GET['halaman'] ?? 1));
$search_query = trim($_GET['cari'] ?? '');
$hadith_per_page = 10;

// Validate book selection
if (!array_key_exists($selected_book, $available_books)) {
    $selected_book = 'bukhari';
}

// Fetch hadith data
function fetchHadiths($book, $page, $per_page) {
    global $hadith_api_base;
    $start = ($page - 1) * $per_page + 1;
    $end = $start + $per_page - 1;
    $url = $hadith_api_base . $book . '?range=' . $start . '-' . $end;
    ...
    return $data['data']['hadiths'] ?? null;
}

$total_hadith = $available_books[$selected_book]['total'];
$total_pages = ceil($total_hadith / $hadith_per_page);
$current_page = min($current_page, $total_pages);
$hadith_data = fetchHadiths($selected_book, $current_page, $hadith_per_page);
?>
...
<head>
    ...
    <link rel="stylesheet" href="../assets/css/hadits.css">
...
<body>
    ...
    <!-- Main Page -->
        ...
        <section>
            <div class="container" style="padding-top: 120px;;">
                <div class="header">
                    <h1>Kumpulan Hadits Shahih</h1>
                    <p>Temukan dan pelajari hadits-hadits shahih dari berbagai kitab hadits utama</p>
                </div>
                ...
                <div class="hadith-container">
                    ...
                    <div class="sidebar">
                        <div class="search-box">
                            ...
                            <form action="" method="get">
                                <input type="hidden" name="kitab" value="<?= $selected_book ?>">
                                <input type="text" class="search-input" name="cari" placeholder="Cari hadits..." 
                        ...
                        <h2 class="sidebar-title">
                            <i class="fas fa-book-quran"></i> Daftar Kitab
                        </h2>
                        <ul class="book-list">
                            <?php foreach ($available_books as $key => $book): ?>
                                <li class="book-item">
                                    <a href="?kitab=<?= $key ?>&halaman=1" 
                                    class="book-link <?= $selected_book == $key ? 'active' : '' ?>">
                    ...
                    <div class="hadith-content">
                        ...
                        <div class="hadith-header">
                            <h2 class="hadith-title"><?= $available_books[$selected_book]['name'] ?></h2>
                            <div class="hadith-meta">
                                <span class="meta-item">
                                    <i class="fas fa-hashtag"></i> Halaman <?= $current_page ?> dari <?= $total_pages ?>
                                </span>
                                <span class="meta-item">
                                    <i class="fas fa-book"></i> Total: <?= $total_hadith ?> Hadits
                                </span>
                            </div>
                        </div>

                        <?php if (!empty($hadith_data)): ?>
                            <div class="hadith-list">
                                <?php foreach ($hadith_data as $hadith): ?>
                                    <div class="hadith-card" id="hadith-<?= $hadith['number'] ?>">
                                        <span class="hadith-number">Hadits No. <?= $hadith['number'] ?></span>
                                        ...
                                        <div class="hadith-translation">
                                            <h3 style="margin-bottom: 10px; color: var(--islamic-green);">Terjemahan:</h3>
                                            <?= $hadith['id'] ?>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                        ...
                        <?php else: ?>
                            <div class="error-message">
                                ...
                                <h3>Hadits tidak ditemukan</h3>
                                <p style="margin-top: 10px;">Silakan coba dengan kata kunci atau halaman yang berbeda</p>
                            </div>
                        <?php endif; ?>
                        ...
                        <div class="pagination">
                            <?php if ($current_page > 1): ?>
                                <a href="?kitab=<?= $selected_book ?>&halaman=1<?= $search_query ? '&cari='.urlencode($search_query) : '' ?>" 
                                class="page-link" title="Halaman Pertama">
                                    <i class="fas fa-angle-double-left"></i>
                                </a>
                                <a href="?kitab=<?= $selected_book ?>&halaman=<?= $current_page - 1 ?><?= $search_query ? '&cari='.urlencode($search_query) : '' ?>" 
                                class="page-link" title="Halaman Sebelumnya">
                            ...
                            for ($i = $start_page; $i <= $end_page; $i++): ?>
                                <a href="?kitab=<?= $selected_book ?>&halaman=<?= $i ?><?= $search_query ? '&cari='.urlencode($search_query) : '' ?>" 
                            ...
                            <?php if ($current_page < $total_pages): ?>
                                <a href="?kitab=<?= $selected_book ?>&halaman=<?= $current_page + 1 ?><?= $search_query ? '&cari='.urlencode($search_query) : '' ?>" 
                                class="page-link" title="Halaman Selanjutnya">
                                    <i class="fas fa-angle-right"></i>
                                </a>
                                <a href="?kitab=<?= $selected_book ?>&halaman=<?= $total_pages ?><?= $search_query ? '&cari='.urlencode($search_query) : '' ?>" 
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const searchQuery = urlParams.get('cari');
            ...
            if (window.location.hash) {
                scrollToHadith(window.location.hash);
            }
        });
        ...
        function highlightText(searchQuery) {
            ...
            const hadithElements = document.querySelectorAll('.hadith-arabic, .hadith-translation');
            
            hadithElements.forEach(element => {
        ...
        function scrollToHadith(hash) {
            const hadithElement = document.querySelector(hash);
            if (hadithElement) {
                setTimeout(() => {
    ...
    <!-- <script>
        // Hadith Data Loader
        let currentPage = 1;
        const hadithPerPage = 9;

        async function loadHadith(page = 1, kitab = 'all', kategori = 'all', search = '') {
            $('#hadith-grid').html(`
                <div class="loading-hadith">
            ...
            try {
                ...
                const response = await mockApiCall(page, kitab, kategori, search);
                displayHadith(response.hadiths);
                
                // Update pagination
                $('#page-info').text(`Halaman ${page}`);
                $('#prev-page').prop('disabled', page <= 1);
                $('#next-page').prop('disabled', page * hadithPerPage >= response.total);
                
            } catch (error) {
                $('#hadith-grid').html(`
                    <div class="error-message">
                        <i class="fas fa-exclamation-triangle"></i>
                        <p>Gagal memuat hadits. Silakan coba lagi.</p>
                        <button onclick="loadHadith(currentPage)">Muat Ulang</button>
                    </div>
                `);
            }
        }
        ...
        function displayHadith(hadiths) {
            if (hadiths.length === 0) {
                $('#hadith-grid').html(`
                    <div class="no-results">
                        ...
                        <p>Tidak ditemukan hadits yang sesuai</p>
                    </div>
                `);
                return;
            }
            ...
            hadiths.forEach(hadith => {
                ...
                <div class="hadith-card" data-id="${hadith.id}">
                    <div class="card-header">
                        <span class="kitab-badge" data-kitab="${hadith.kitab}">${hadith.kitab}</span>
                        <span class="hadith-number">#${hadith.number}</span>
                    </div>
                    <div class="card-body">
                        <div class="hadith-text">${hadith.translation.substring(0, 150)}...</div>
                        <div class="hadith-category">
                            <span class="category-tag" data-category="${hadith.category}">${hadith.category}</span>
                        </div>
                    </div>
                    <div class="card-footer">
                        <button class="view-detail" data-id="${hadith.id}">
                            <i class="fas fa-eye"></i> Baca Lengkap
                        </button>
                    </div>
                </div>
                `;
            ...
            $('#hadith-grid').html(html);
            
            // Add click handlers
            $('.view-detail').click(function() {
                const hadithId = $(this).data('id');
                showHadithDetail(hadithId);
            });
        }

        function showHadithDetail(hadithId) {
            // In real implementation, fetch full hadith data
            const hadith = {
                id: hadithId,
                arabic: "إنما الأعمال بالنيات وإنما لكل امرئ ما نوى...",
                translation: "Sesungguhnya amal itu tergantung niatnya, dan setiap orang akan mendapatkan sesuai dengan apa yang dia niatkan...",
                kitab: "Shahih Bukhari",
                number: "1",
                category: "Aqidah"
            };
            
            $('#modal-title').text(hadith.kitab);
            $('#modal-arabic').html(hadith.arabic);
            $('#modal-translation').text(hadith.translation);
            $('#modal-kitab').text(hadith.kitab);
            $('#modal-number').text(`Hadits No. ${hadith.number}`);
            
            $('#hadith-modal').fadeIn();
        }

        // Mock API function - replace with real API call
        async function mockApiCall(page, kitab, kategori, search) {
            return new Promise(resolve => {
                setTimeout(() => {
                    const mockHadiths = [];
                    const kitabs = ['Shahih Bukhari', 'Shahih Muslim', 'Sunan Tirmidzi'];
                    const categories = ['Aqidah', 'Ibadah', 'Muamalah', 'Akhlak'];
                    
                    for (let i = 1; i <= hadithPerPage; i++) {
                        const randomKitab = kitabs[Math.floor(Math.random() * kitabs.length)];
                        const randomCategory = categories[Math.floor(Math.random() * categories.length)];
                        
                        if ((kitab === 'all' || randomKitab.toLowerCase().includes(kitab)) &&
                            (kategori === 'all' || randomCategory.toLowerCase() === kategori)) {
                            mockHadiths.push({
                                id: (page - 1) * hadithPerPage + i,
                                kitab: randomKitab,
                                number: (page - 1) * hadithPerPage + i,
                                translation: "Ini adalah contoh teks hadits tentang " + 
                                            ["iman", "shalat", "zakat", "puasa", "akhlak"][Math.floor(Math.random() * 5)] + 
                                            ". Rasulullah ﷺ bersabda tentang pentingnya hal ini dalam Islam.",
        ...
        $(document).ready(function() {
            ...
            // Filters
            $('#kitab-filter, #kategori-filter').change(function() {
                currentPage = 1;
                loadHadith(
                    currentPage,
                    $('#kitab-filter').val(),
            ...
            $('#hadith-search').keypress(function(e) {
                if (e.which === 13) {
                    currentPage = 1;
                    loadHadith(
                        currentPage,
            ...
            // Reset filters
            $('#reset-filters').click(() => {
                $('#kitab-filter, #kategori-filter').val('all');
                $('#hadith-search').val('');
                currentPage = 1;
                loadHadith();
            });
            ...
            // Modal controls
            $('#close-modal').click(() => {
                $('#hadith-modal').fadeOut();
            });
            
            $(document).click((e) => {
                if ($(e.target).is('#hadith-modal')) {
                    $('#hadith-modal').fadeOut();
                }
            });
            ...
            // Copy hadith
            $('#copy-hadith').click(() => {
                const textToCopy = $('#modal-arabic').text() + "\n\n" + $('#modal-translation').text();
                navigator.clipboard.writeText(textToCopy);
                alert('Hadits telah disalin!');
            });
    ...
    <!-- <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Highlight search terms in hadith text
                ...
                if (searchTerms.length > 0) {
                    ...
                    hadithElements.forEach(element => {
                        ...
                        searchTerms.forEach(term => {
                            const regex = new RegExp(term, 'gi');
                            html = html.replace(regex, match => 
                                `<span style="background-color: #FFEB3B; padding: 0 2px;">${match}</span>`
                            );
                        });
                        element.innerHTML = html;
                    });
            ...
            // Jump to specific hadith if hash is present in URL
            if (window.location.hash) {
                const hadithElement = document.querySelector(window.location.hash);
                if (hadithElement) {
                    setTimeout(() => {
                        hadithElement.scrollIntoView({ behavior: 'smooth' });
                    }, 500);
