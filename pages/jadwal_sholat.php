...
$defaultCity = "Jakarta";
$defaultMethod = 1; // 1 for Kemenag, 2 for Muslim World League, etc.

// Get user location if available
$latitude = $_GET['lat'] ?? $defaultLatitude;
$longitude = $_GET['lng'] ?? $defaultLongitude;
$city = $_GET['city'] ?? $defaultCity;
$method = $_GET['method'] ?? $defaultMethod;

// Prayer times calculation (using dummy data - replace with actual API call)
function getPrayerTimes($lat, $lng, $method) {
...
function getHijriDate($gregorianDate) {
    ...
    $day = $gregorianDate->format('j');
    $month = $months[(int)$gregorianDate->format('n') - 1];
    $year = (int)$gregorianDate->format('Y') - 579;
    
    return "$day $month $year H";
}

$prayerTimes = getPrayerTimes($latitude, $longitude, $method);
$currentPrayer = 'Dhuhr'; // In real app, calculate current prayer time
?>
...
<head>
    ...
    <link rel="stylesheet" href="../assets/css/jadwal_sholat.css">
...
<body>
    ...
    <!-- Main Page -->
        ...
        <section>
            <div class="container" style="padding-top: 120px;">
                <div class="header">
                    <h1>Jadwal Sholat</h1>
                    <p>Waktu sholat harian berdasarkan lokasi Anda</p>
                </div>

                <div class="date-display">
                    <div class="gregorian"><?= $prayerTimes['date'] ?></div>
                    <div class="hijri"><?= $prayerTimes['hijri'] ?></div>
                </div>

                <div class="location-selector">
                    <div class="location-info">
                        <h3><?= htmlspecialchars($city) ?></h3>
                        <p>Koordinat: <?= round($latitude, 4) ?>, <?= round($longitude, 4) ?></p>
                    </div>
                    <div class="location-actions">
                        <button class="btn btn-primary" id="change-location">
                            <i class="fas fa-map-marker-alt"></i> Ubah Lokasi
                        </button>
                        <button class="btn btn-outline" id="detect-location">
                ...
                <div class="prayer-times">
                    <?php foreach ($prayerTimes as $prayer => $time): ?>
                        <?php if (!in_array($prayer, ['date', 'hijri'])): ?>
                            <div class="prayer-card <?= $prayer === $currentPrayer ? 'current' : '' ?>">
                                <div class="prayer-name"><?= $prayer ?></div>
                                <div class="prayer-time"><?= $time ?></div>
                                <?php if ($prayer === $currentPrayer): ?>
                                    <div class="prayer-remaining">Waktu sholat saat ini</div>
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
                    <?php endforeach; ?>
                ...
                <div class="iqamah-times">
                    <h3><i class="fas fa-clock"></i> Waktu Iqamah</h3>
                    <div class="iqamah-grid">
                        <div class="iqamah-item">
                            <div class="prayer-name">Subuh</div>
                            <div class="iqamah-time">04:45</div>
                        </div>
                        <div class="iqamah-item">
                            <div class="prayer-name">Dzuhur</div>
                            <div class="iqamah-time">12:15</div>
                        </div>
                        <div class="iqamah-item">
                            <div class="prayer-name">Ashar</div>
                            <div class="iqamah-time">15:30</div>
                        </div>
                        <div class="iqamah-item">
                            <div class="prayer-name">Maghrib</div>
                            <div class="iqamah-time">5 menit setelah adzan</div>
                        </div>
                        <div class="iqamah-item">
                            <div class="prayer-name">Isya</div>
                            <div class="iqamah-time">19:20</div>
                        </div>
                    </div>
                </div>

                <div class="next-prayer">
                    <i class="fas fa-info-circle"></i> Sholat berikutnya: Ashar pukul <?= $prayerTimes['Asr'] ?>
                </div>
            </div>
                ...
                <div class="modal-content">
                    ...
                    <form id="location-form">
                        <div class="form-group">
                            ...
                            <input type="text" id="city" class="form-control" value="<?= htmlspecialchars($city) ?>">
                        </div>
                        <div class="form-group">
                            <label for="latitude">Latitude</label>
                            <input type="text" id="latitude" class="form-control" value="<?= $latitude ?>">
                        </div>
                        <div class="form-group">
                            <label for="longitude">Longitude</label>
                            <input type="text" id="longitude" class="form-control" value="<?= $longitude ?>">
                        </div>
                        <div class="form-group">
                            <label for="method">Metode Perhitungan</label>
                            <select id="method" class="form-control">
                                <option value="1" <?= $method == 1 ? 'selected' : '' ?>>Kemenag RI</option>
                                <option value="2" <?= $method == 2 ? 'selected' : '' ?>>Muslim World League</option>
                                <option value="3" <?= $method == 3 ? 'selected' : '' ?>>Islamic Society of North America</option>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const openModalBtn = document.getElementById('change-location');
        ...
        detectLocationBtn.addEventListener('click', () => {
            if (navigator.geolocation) {
                detectLocationBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Mendeteksi...';
                ...
                navigator.geolocation.getCurrentPosition(
                    (position) => {
                        ...
                        window.location.href = `?lat=${lat}&lng=${lng}&city=Lokasi+Anda`;
                    },
                    (error) => {
                        alert('Gagal mendeteksi lokasi: ' + error.message);
                        detectLocationBtn.innerHTML = '<i class="fas fa-location-arrow"></i> Deteksi Lokasi';
                    }
                );
            } else {
        ...
        document.getElementById('location-form').addEventListener('submit', function(e) {
            ...
            window.location.href = `?lat=${lat}&lng=${lng}&city=${encodeURIComponent(city)}&method=${method}`;
        });
        
        // Update prayer times every minute
        setInterval(() => {
            // In a real app, you would recalculate prayer times
